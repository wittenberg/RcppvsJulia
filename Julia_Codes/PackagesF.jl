...
#--- Manual Functions 
	...
	# Function for 'Code5.jl'
		function ciallf(x::Int64, z::Float64, n::Int64, 
					...
					# Wald
						# ciwa = confint(BinomialTest(x, n, p), level = (1 - α),  method = :wald)
						c0 = x/n;
						c1 = z*sqrt(c0*(1 - c0)/n);

					# Wilson
						# ciwi = confint(BinomialTest(x, n, p), level = (1 - α),  method = :wilson)
						b0  = (1/(1+ z^2/n))*(c0 + (z^2)/(2*n))
						b1  = (z/(1 + (z^2/n)))*sqrt((c0*(1 - c0)/n) + z^2/(4*n^2)  )

					# Return CIs
						SVector{3, Bool}((c0 - c1)     <= p <= (c0 + c1), # ciwa[1]  <= p <=  ciwa[2], #
														 (b0 - b1)  	 <= p <= (b0 + b1), # ciwi[1]  <= p <=  ciwi[2], # 
